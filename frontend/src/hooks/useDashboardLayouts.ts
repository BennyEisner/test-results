...
export const useDashboardLayouts = () => {
  ...
  const saveLayouts = (newLayouts: DashboardLayout[], newActiveId: string) => {
    ...
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ layouts: newLayouts, activeLayoutId: newActiveId }));
  };
  ...
  const removeComponent = (componentId: string) => {
    ...
    updateLayout({
      ...
      gridLayout: activeLayout.gridLayout.filter(item => item.i !== componentId),
    });
  };
